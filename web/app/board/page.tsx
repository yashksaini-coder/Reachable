...
export default async function Board() {
  ...
  return (
    <div className="space-y-4">
      ...
      {cards.length === 0 && (
        <div className="panel px-5 py-8 text-center text-[13px] text-ink-2">No incidents composed yet.</div>
      )}
      <div className="grid gap-3 lg:grid-cols-5">
        {COLUMNS.map((col) => {
          const list = byState.get(col.key)!;
          return (
            <section key={col.key} className={`panel border-t-2 ${col.tone} p-2`}>
              <div className="flex items-baseline justify-between px-1 pb-2">
                <div>
                  <div className="text-[13px] font-medium">{col.title}</div>
                  <div className="text-[10.5px] text-ink-3">{col.hint}</div>
                </div>
                <div className="num text-lg text-ink-2">{list.length}</div>
              </div>
              <div className="space-y-2">
                {list.map((c) => (
                  <Link
                    ...
                    className="panel-2 block px-3 py-2 hover:border-line-2"
                  >
                    <div className="truncate font-mono text-[12.5px]">{svcSlug(c.service)}</div>
                    <div className="mt-1 flex flex-wrap items-center gap-1.5">
                      <Level level={c.level} />
                      {c.whileLive && <span className="chip chip-l1">while live</span>}
                      <span className="chip">{c.advisory}</span>
                    </div>
                ...
                {list.length === 0 && <div className="px-1 py-4 text-center text-[11px] text-ink-3">—</div>}
              </div>
            </section>
          );
        })}
