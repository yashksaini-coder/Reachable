    ...
    <div className="space-y-10">
      <header className="space-y-4">
        ...
        <div className="flex flex-wrap items-baseline gap-3">
          ...
          <a href={advisoryUrl(inc.advisory.key)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground">
            source <ExternalLink className="size-3" />
          </a>
        </div>
        <p className="max-w-3xl text-[14px] text-foreground/90">{inc.advisory.summary}</p>
                ...
                return (
                  <TableRow key={svc}>
                    <TableCell className="font-mono">
                      <Link href={`/incident/${inc.advisory.key}/${svcSlug(svc)}`} className="hover:text-signal-2">
                        {svcSlug(svc)}
                      </Link>
                    </TableCell>
                    <TableCell>
      ...
      <section>
        ...
        {q3 === null ? (
          <div className="rounded-lg border border-border bg-card p-4 text-sm text-muted-foreground">
            Not applicable: this is a CVE — the artifact is still on the registry, so exposure is not time-bounded and
            “resolved while live” collapses into “resolved at all” (question 1).
          </div>
        ) : (
          <>
              ...
              <Table>
                ...
                <TableBody>
                  {q3.rows.map((r, i) => (
                    <TableRow key={i}>
                      <TableCell className="font-mono">
                        <Link href={`/incident/${inc.advisory.key}/${svcSlug(r.service)}`} className="hover:text-signal-2">
                          {svcSlug(r.service)}
                        </Link>
                      </TableCell>
                      <TableCell className="font-mono text-xs">
