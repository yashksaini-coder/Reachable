...
export default function Pipeline() {
  ...
  return (
    <Svg id={id} title="Ingest pipeline: GitHub lockfile history, the npm registry, api.npmjs.org downloads and OSV advisories feed the worker stages lockfiles, packages, advisories and reach, which write the HydraDB node labels and relationships" h={400}>
      <text x={xS} y={30} textAnchor="middle" className="fill-dim text-[10.5px] uppercase tracking-[0.08em]">sources</text>
      <text x={xW} y={30} textAnchor="middle" className="fill-dim text-[10.5px] uppercase tracking-[0.08em]">worker stages</text>
      <text x={xO} y={30} textAnchor="middle" className="fill-dim text-[10.5px] uppercase tracking-[0.08em]">HydraDB · labels written</text>

      {SRC.map((s) => (
        <g key={s.label}>
          <Box cx={xS} cy={s.y} w={170} h={44} label={s.label} />
          <text x={xS} y={s.y + 34} textAnchor="middle" className="fill-dim text-[10.5px]">{s.sub}</text>
          {s.to.map((t) => (
      ...
      {STAGE.map((s, i) => (
        <g key={s.label}>
          ...
          <text x={xW} y={s.y + 34} textAnchor="middle" className="fill-dim text-[10.5px]">{s.sub}</text>
          ...
          <text x={xO} y={s.y - 3} textAnchor="middle" className="fill-fg font-mono text-[12px]">{OUT[i].labels}</text>
          <text x={xO} y={s.y + 12} textAnchor="middle" className="fill-signal-2 font-mono text-[10.5px]">{OUT[i].edges}</text>
        </g>
      ))}
